<?php
	...
	if(!isset($_FILES['myFile'])) die("파일이 보내지지 않았습니다.");
	...
	//파일 이동
	if(!move_uploaded_file($temp_name, $location.$file_name)) die("업로드 실패!");
	...
	else
	{
		...
		$result = mysqli_query($connect, $sql) or die("에러 발생-room_id");
		$count = mysqli_num_rows($result);
		if($count!=1) die("존재하지 않는 room_id 입니다.");

		//데이터베이스에 file_path 중복 확인
		$sql = "SELECT file_path FROM video_data WHERE file_path = '{$file_path}'";
		$result = mysqli_query($connect, $sql) or die("에러 발생-file_path");
		$count = mysqli_num_rows($result);
		if($count==1) die("중복되는 file 입니다.");
		else die("알수없는 에러");
	}
